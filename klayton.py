...
import os, re, csv, sys, math, errno, signal, socket, timeit, datetime, platform, threading, xml.parsers.expat
    ...
    def pre_allocate(self):
        ...
        try:
            ...
            raise SpeedtestCLIError(
                'Memoria insuficiente. Por favor '
                'use --no-pre-allocate'
            )
...
class Speedtest(object):
    ...
    def set_mini_server(self, server):
        ...
        uh, e = catch_request(request, opener=self._opener)
        if e:
            raise SpeedtestMiniConnectFailure('Falha ao conectar %s' %
                                              server)
        else:
            text = uh.read()
            uh.close()
        ...
        if not urlparts or not extension:
            raise InvalidSpeedtestMiniServer('Servidor Invalido: '
                                             '%s' % server)
    ...
    def get_best_server(self, servers=None):
        """Perform a "ping" to determine which speedtest.net
        ...
        try:
            ...
            raise SpeedtestBestServerFailure('Não foi possivel conectar ao servidor de teste'
                                             'Latencia.')
...
def parse_args():
    ...
    description = (
        'Command line interface for testing internet '
        '.\n'
        '------------------------------------------------------------'
        '--------------\n'
        '')
...
==                     T! SOS Sistems                      ==
=============================================================
Verificador de velocidade de internet,  objetivo  facilitar no 
laudo e  constatação  da real  velocidade da internet ofertada
pelos provedores ISP

Data do teste:{} 
Perito: Klayton Bueno Prince'''.format(dia), quiet)
    try:
        speedtest = Speedtest(
        ...
        printer('Não foi póssivel conectar ao servidor de teste', error=True)
    ...
    printer('Auditando provedor de internet:%(isp)s  Seu IP:(%(ip)s)' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('''Aguarde, estamos concluindo os testes''', quiet)
        try:
            speedtest.get_servers(servers=args.server, exclude=args.exclude)
        except NoMatchedServers:
            raise SpeedtestCLIError(
                'Não foi póssivel conectar ao servidor de teste: %s' %
        ...
        if args.server and len(args.server) == 1:
            printer('Não foi póssivel conectar ao servidor de teste...', quiet)
        else:
            printer('Verificando o melhor servidor por base no teste de ping...', quiet)
    ...
    printer('Servidor de teste hospedado por  %(sponsor)s (%(name)s) Distancia:%(d)0.2f km: '
            'Latencia:%(latency)s ms' % results.server, quiet)

    if args.download:
        printer('Verificando velocidade de download', quiet,
                end=('', '\n')[bool(debug)])
    ...
    if args.share and not machine_format:
        printer('Compartilhar resultado: %s' % results.share())
